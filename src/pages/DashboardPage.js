...
export default function DashboardPage() {
    ...
    /** 🔹 Remove display */
    const handleRemoveFromDisplay = async (index) => {
    ...
    /** 🔹 UI Layout */
    return (_jsxs("div", { className: "min-h-screen bg-gray-50", children: [_jsx(Sidebar, { onWidthChange: setSidebarWidth }), _jsxs("main", { className: "transition-all duration-300 overflow-y-auto", style: { marginLeft: `${sidebarWidth}px` }, children: [_jsx(Navbar, { title: "Dashboard" }), _jsx("div", { className: "px-6 py-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-8", children: cards.map((card, i) => (_jsxs("div", { className: "rounded-xl bg-white shadow-sm p-3 flex flex-col items-center", children: [_jsxs("div", { className: "flex items-center justify-between w-full mb-3 px-1", children: [_jsxs("h3", { className: "text-sm font-semibold text-gray-700", children: ["Slot ", i + 1] }), _jsx("span", { className: `text-xs font-medium px-2 py-1 rounded-full ${card?.customer_name
                                                ? 'bg-green-100 text-green-700'
                                                : 'bg-gray-100 text-gray-500'}`, children: card?.customer_name ? 'Active' : 'Inactive' })] }), _jsx(TVDisplayCard, { data: card && card.customer_name
                                        ? card
                                        : {
                                            customer_name: '',
                                            brand: '',
                                            ...
                                            year: '',
                                            estimated_time: '',
                                        }, index: i, onRemove: handleRemoveFromDisplay })] }, i))) })] })] }));
}
